...
Espaciado:
- `--gutter: 30px`: margen lateral, separación entre columnas y entre bloques, y padding de las cajas. En la home también es la única separación vertical entre secciones (desktop y mobile): no usar 40, 60 ni 90px entre secciones. El contenido ocupa todo el ancho de la ventana (sin max-width).
...
Breakpoints:
- Las pills del hero (`.pill-row`) reparten el espacio entre ellas según el ancho (`space-between`). Desde 1100px hacia abajo van en 2 filas: 4 arriba y el resto abajo (con un salto de línea invisible, `.pill-row::after`), cada fila de borde a borde. Desde 640px hacia abajo fluyen de izquierda a derecha con separación fija, porque los textos son demasiado largos para filas parejas. Cada pill conserva el ancho de su texto.
...
**Home:** orden de lectura: hero (valor + pills de áreas), Work, Background / How I work, "The kind of problems I like working on" (`.problems`: una caja blanca con el mismo padding y radio que las otras cajas, con el título y los cuatro problemas adentro en bloques 2 × 2 blancos con borde negro de 1px, radio 20px, padding y gap de 20px, sin sombras; número chico arriba a la izquierda y texto en `--fs-h4`; una columna en mobile), Playground (`#experiments`: directamente sobre el fondo, sin caja; label, intro a todo el ancho y los tres proyectos; en desktop la intro escala con `min(vw, 32px)` para quedar en dos líneas), How I got here (`.experience-intro`: bloque lavanda con el label y el párrafo; el timeline queda afuera, sobre el fondo), personal, contacto. Las secciones de texto van dentro de cajas con fondo (lavender, lime o blanco). El timeline de experiencia usa la retícula de 12 columnas: fecha (columnas 1-3), puesto, empresa y descripción (4-10) y metadatos (11-12) como texto chico en gris, uno por línea y alineados a la derecha, sin pills (`.timeline-meta`). Desde 1100px hacia abajo los metadatos pasan debajo del contenido, en una línea. Las cards de proyectos muestran al hacer hover un degradado oscuro, el título del case study (el mismo H1 de su página y de `PROJECTS` en `script.js`, en `--fs-body`) en blanco y pills blancas con los servicios, alineados abajo. En pantallas táctiles (`hover: none`) se muestran siempre.
...
- Tooltips: componente CSS reutilizable, se activa con `data-tooltip="Texto"` (y `data-tooltip-pos="bottom"` para mostrarlo abajo). Aparece con hover (solo en dispositivos con mouse) y con `focus-visible`. No usar `title=""`. Textos: "Copiar", "Abrir LinkedIn", "Abrir Medium", "Abrir CV".
